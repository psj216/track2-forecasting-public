...
from backtesting.build_evidence_replay import build_evidence_replay
...
def test_scenario_backtest_compares_candidate_with_identical_numeric_draws(
    ...
    detail = run_scenario_backtest(
        ...
        records,
        500,
        configs_by_family={"T2-F1": (config,)},
    )
...
""",
    ...
    skipped = ReasoningResult(False, "MODEL_ENDPOINT is unset", None, {}, corpus, "")
    monkeypatch.setenv("MODEL_ENDPOINT", "https://organizer.invalid/model")
    ...
    with pytest.raises(RuntimeError, match="no evidence replay cases"):
        build_evidence_replay(tmp_path, tmp_path / "replay.jsonl", 1, 200)
